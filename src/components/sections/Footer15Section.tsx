        ...
        <footer className="w-full py-16" style={{ background: backgroundColor, color: textColor }}>
            <div className="container mx-auto px-4">
                <div className="text-center mb-12">
                    <Link href="/">{logo ? <img src={logo} alt={logoText} className="h-12 w-auto mx-auto mb-6" /> : <span className="text-3xl font-bold block mb-6">{logoText}</span>}</Link>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                        {cards.map((card: any, i: number) => (
                            <div key={i} className="p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow" style={{ background: cardBgColor }}>
                                ...
                                {card.link && (
                                    <Link href={card.link} className="text-sm font-bold hover:underline">
