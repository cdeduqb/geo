...
async function scoreWithAI(title: string, content: string): Promise<any> {
    try {
        // 优先获取文本生成模型（非绘画模型）
        // 优先级: openai > deepseek > 其他文本模型 > volcengine
        ...
        // 检查是否是绘画模型（通过模型名称判断）
        const imageModelKeywords = ['seedream', 'dall-e', 'midjourney', 'stable-diffusion', 'image', 'paint', 'draw'];
        const isImageModel = imageModelKeywords.some(keyword =>
            aiConfig!.modelName?.toLowerCase().includes(keyword)
        );

        if (isImageModel) {
            console.warn('[AI评分] 检测到图像生成模型，可能不支持文本对话:', aiConfig.modelName);
            console.warn('[AI评分] 建议配置文本对话模型如: gpt-3.5-turbo, deepseek-chat, doubao-pro-32k');
        }
